...
void GPIO_Init(GPIO_Handle_t *pGPIOHandle){
    ...
    if(pGPIOHandle->GPIO_PinConfig.GPIO_PinMode <= GPIO_MODE_OUT_50mhz) //GPIO_MODE_OUT_2mhz value is 0b11 all another gpio cofigurations are less than 0b11
    {
    ...
    /*
    uint32_t modeMask = 0x3 << (4 * temp2);  // Mask to clear the mode bits for the specific pin
    pGPIOHandle->pGPIOx->CR[temp1] &= ~modeMask;  // Clear mode bits
    pGPIOHandle->pGPIOx->CR[temp1] |= (pGPIOHandle->GPIO_PinConfig.GPIO_PinMode << (2 * temp2));  // Set the mode bits

    uint32_t confMask = 0x3 << (2 * temp2);  // Mask to clear the configuration bits for the specific pin
    pGPIOHandle->pGPIOx->CR[temp1] &= ~confMask;  // Clear configuration bits
    pGPIOHandle->pGPIOx->CR[temp1] |= (pGPIOHandle->GPIO_PinConfig.GPIO_PinConf << (2 * temp2));  // Set the configuration bits
